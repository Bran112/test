...
void EntityManager::update() {
  for (auto entities : m_toAdd) {
    m_entities.push_back(entities);
    m_entityMap[entities->tag()].push_back(entities);
  }
  m_toAdd.clear();
...
std::shared_ptr<Entity> EntityManager::addEntity(const std::string &tag) {
  m_totalEntities++;
  auto my_pointer = std::shared_ptr<Entity>(new Entity(m_totalEntities, tag));
  m_toAdd.push_back(my_pointer);
  return my_pointer;
}
